<template>
  <div class="m-topic">
    <h2>{{ title }}</h2>
    <div
      class="m-topic__organisation"
      v-for="(organisation, index) in organisations"
      :key="organisation.node.id"
    >
      <h3>
        <g-link :to="organisation.node.address">{{ organisation.node.name }}</g-link>
      </h3>
      <Timeframe
        :dateFrom="organisation.node.dateFrom"
        :dateTo="organisation.node.dateTo"
        v-if="organisation.node.events.edges.length > 1"
      />
