...
async function savedDotIoScraper() {
  ...
  await Promise.all([page.click('form.login input[type=submit]'), page.waitForNavigation()])
...
async function savedDotIoParser(scaperResults) {
  for (let rawBookmarkObj of scaperResults) {
    ...
    let bookmarkSubDirectory = `${pathBase}/content/bookmarks/${bookmarkPublishedDateObj.getFullYear()}-${padString(
      bookmarkPublishedDateObj.getMonth() + 1
    )}`
...
function getFirstDayOfMonthUnixtime(dateObj, timeZone) {
  let firstDayCurrentMonth = new Date(dateObj.getFullYear(), dateObj.getMonth(), 1)
...
savedDotIoParser(await savedDotIoScraper())
